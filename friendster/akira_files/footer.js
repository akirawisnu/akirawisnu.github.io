...
function refreshGoogleAd() {
	...
	if ((okToRefreshIframeAd[adID] || !refreshIframeAdTimer[adID])) {
		if(window.frames&&window.frames["google_ads_frame"]&&window.google_ad_url) {
			var adUrl=window.google_ad_url.replace(/&dt=(\d+)&/,"&dt="+new Date().getTime()+"&");
			window.frames["google_ads_frame"].location.replace(adUrl);
