...
var Locale = {
    ...
    getLocale : function() {
        try {
            var curLoc = Cookie.get('friendster_lang');
        } catch(e) {
            return 'en-US';    
        }
    ...
    getString : function(name, locale) {
        try {
            if(!locale) var locale = Locale.getLocale();
            return (!Locale[locale]) 
                ? ((!Locale['en-US'][name]) ? '' : Locale['en-US'][name])
  ...
  var ultags=document.getElementById(menuids[i]).getElementsByTagName("ul")
    for (var t=0; t<ultags.length; t++){
        ...
        ultags[t].parentNode.onmouseover=function(){
			...
			this.getElementsByTagName("a")[0].style.backgroundImage="url(http://images.friendster.com/images/arrow_down.gif)";
...
function changeSurferList(mainURL, newSurfer)
{
    sendXMLHttpRequestText(mainURL+'/surfer.php?surfer='+newSurfer, 'handleSurferListChange');
}

function handleSurferListChange(html)
{
    if (html)
    {
        var json = eval("(" + html + ")");

		var next = document.getElementById('surfer_next');
		var nextBox = document.getElementById('surferNxt');
	    if (next != null)
	    {
			next.setAttribute('href', json.mainUrl + '/surfer.php?action=next');
			if (json.hasNext == 'n') {
				next.removeAttribute('href');
				nextBox.className ="nxtBtnDisb";
			}
			else {
				nextBox.className ="nxtBtn";
			}
	    }

		var prev = document.getElementById('surfer_prev');
		var prevBox = document.getElementById('surferPrv');
	    if (prev != null)
	    {
			prev.setAttribute('href', json.mainUrl + '/surfer.php?action=prev');
			if (json.hasPrev == 'n') {
				prev.removeAttribute('href');
				prevBox.className ="backBtnDisb";
			}
			else {
				prevBox.className ="backBtn";
			}
	    }
    }
}
